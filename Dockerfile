...
# amd64 or arm64, there is no armv7 build.
...
# The database is a bind mount, never baked into the image. /data is only a
# mountpoint: the host directory carries the ownership (uid 1000). No RUN step
# anywhere, so the arm64 image cross-builds without qemu.
ENV TABLE_SIX_DB=/data/table-six.db
VOLUME /data

# The server derives "today" from the local timezone (src/dates.ts) and a
# container is UTC unless told otherwise. Override it where it is deployed.
